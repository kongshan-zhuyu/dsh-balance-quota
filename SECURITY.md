...
- 余额接口和用户配置的外部状态 API 允许公网 HTTP 或 HTTPS，拒绝回环、私网、链路本地和内部域名。
  允许明文 HTTP 是为了覆盖「网关部署在无域名裸 IP 上、无法签发证书」的场景；SSRF 防线不因此放宽：
  仍强制协议白名单（仅 http/https）、DNS 解析后固定 IP、禁止重定向、拒绝内嵌凭据。
  如需强制 TLS，设置环境变量 `DSH_BALANCE_ALLOW_HTTP=0`。
