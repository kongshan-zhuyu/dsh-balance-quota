...
const DEFAULT_CONFIG = { version: 1, statusBar: true, defaultProviderId: null, bindings: {}, providers: [], externalStatusSources: [] };
...
function sanitizeProviderError(value) {
  return String(value || "").replace(/Bearer\s+[^\s,;]+/gi, "Bearer [redacted]").replace(/((?:api[_-]?key|token|secret|password)\s*[:=]\s*)[^\s,;]+/gi, "$1[redacted]").replace(/[\r\n\t]+/g, " ").trim().slice(0, PROVIDER_ERROR_MAX_LENGTH);
}
...
async function resolvePublicEndpoint(raw) {
...
function requestPinnedJson(provider, headers) {
  return new Promise(async (resolve, reject) => {
    let target;
    try { target = await resolvePublicEndpoint(provider.endpoint); } catch (error) { reject(error); return; }
    ...
    const request = https.request({
      ...
      if (status >= 300 && status < 400) { response.resume(); reject(new HttpError(502, "provider redirect is not allowed")); return; }
      const contentLength = Number(response.headers["content-length"] || 0);
      if (contentLength > MAX_BODY) { response.resume(); reject(new HttpError(502, "provider response too large")); return; }
      const chunks = [];
      let size = 0;
      response.on("data", (chunk) => {
        size += chunk.length;
        if (size > MAX_BODY) { response.destroy(new HttpError(502, "provider response too large")); return; }
        chunks.push(chunk);
      });
...
const EXTERNAL_STATUS_FIELDS = Object.freeze({
  model: "$.model",
  status: "$.status",
...
function safeExternalPath(value, optional = true) { return optional && (value === "" || value === null || value === undefined) ? true : safePath(value); }
function normalizeExternalHealth(value) {
...
function normalizeLatency(value, unit = "ms") {
  const number = Number(value);
  if (!Number.isFinite(number) || number < 0) return undefined;
  return Math.round(unit === "s" ? number * 1000 : number);
}
function readMapped(data, path) { return path ? readJsonPath(data, path) : undefined; }
function normalizeExternalError(value) { return Array.isArray(value) ? value.map(item => String(item || "").trim()).filter(Boolean).join(" / ") : String(value || "").trim(); }
function applyExternalTransform(value, transform) { if (transform === "number") { const number = Number(value); return Number.isFinite(number) ? number : value; } if (transform === "percent") return normalizeAvailability(value); if (transform === "status") return normalizeExternalHealth(value); return value; }
export async function validateExternalStatusSource(input) {
  ...
  const fields = { ...EXTERNAL_STATUS_FIELDS, ...(input.fields && typeof input.fields === "object" ? input.fields : {}) };
   const customFields = Array.isArray(input.customFields) ? input.customFields.slice(0, 50).map(field => { if (!field || typeof field.name !== "string" || !field.name.trim() || field.name.length > 80 || !safeExternalPath(field.path)) throw new Error("invalid external custom field"); return { name: field.name.trim(), path: field.path, transform: ["identity", "number", "percent", "status"].includes(field.transform) ? field.transform : "identity" }; }) : [];
...
export function normalizeExternalStatus(source, payload, now = new Date().toISOString()) {
  ...
  const models = list.slice(0, 500).map((item) => {
    ...
    const history = Array.isArray(rawHistory) ? rawHistory.slice(-EXTERNAL_STATUS_MAX_HISTORY).map(record => ({ at: source.fields.historyAt ? readMapped(record, source.fields.historyAt) : undefined, status: normalizeExternalHealth(source.fields.historyStatus ? readMapped(record, source.fields.historyStatus) : undefined), error: source.fields.error ? normalizeExternalError(readMapped(record, source.fields.error)).slice(0, PROVIDER_ERROR_MAX_LENGTH) : undefined })) : [];
    ...
    return { model: String(applyExternalTransform(readMapped(item, source.fields.model) || "未知模型", transforms.model)).slice(0, 160), status, custom, availability: source.fields.availability ? applyExternalTransform(readMapped(item, source.fields.availability), transforms.availability || "percent") : undefined, ttftMs: source.fields.ttft ? normalizeLatency(applyExternalTransform(readMapped(item, source.fields.ttft), transforms.ttft), source.ttftUnit) : undefined, responseMs: source.fields.response ? normalizeLatency(applyExternalTransform(readMapped(item, source.fields.response), transforms.response), source.responseUnit) : undefined, samples: history.length, history, errors: source.fields.error ? [normalizeExternalError(readMapped(item, source.fields.error)).slice(0, PROVIDER_ERROR_MAX_LENGTH)].filter(Boolean) : [] };
  });
...
async function resolveProviderCredential(provider, credentials) {
  ...
  // Upgrade legacy macOS Keychain data on first use without ever exposing it to
  // the browser or persisting it in the balance configuration file.
...
function externalSourceScore(source) {
  return Number(Boolean(source?.providerId)) * 1000 + Number(Boolean(source?.modelListPath)) * 100 + Object.values(source?.fields || {}).filter(Boolean).length + (Array.isArray(source?.customFields) ? source.customFields.length : 0) + Number(Boolean(source?.preview)) * 10;
}
...
function mutateConfig(mutator) {
  // 所有配置修改串行执行，避免两个保存请求基于同一份旧配置互相覆盖。
...
function previewExternalJson(value, depth = 0) {
  if (depth > 4) return "…";
...
export async function previewExternalStatusSource(input) {
  ...
  let payload; try { payload = JSON.parse(response.text); } catch { throw new HttpError(502, "external status source returned invalid JSON"); }
...
function collectExternalPaths(value, prefix = "$", output = []) {
  if (output.length >= 200) return output;
  if (Array.isArray(value)) { if (value.length) collectExternalPaths(value[0], `${prefix}[]`, output); return output; }
  if (!value || typeof value !== "object") return output;
  for (const [key, item] of Object.entries(value).slice(0, 40)) { const path = `${prefix}.${key}`; output.push({ path, type: Array.isArray(item) ? "array" : item === null ? "null" : typeof item }); collectExternalPaths(item, path, output); }
  return output;
}
async function queryExternalStatus(source, force = false) {
...
async function externalSummary(config, force = false, requestedId = null) {
  const sources = requestedId ? config.externalStatusSources.filter(source => source.id === requestedId) : config.externalStatusSources;
  return Promise.all(sources.map(async source => { try { return await queryExternalStatus(source, force); } catch (error) { return { id: source.id, name: source.name, endpoint: source.endpoint, fetchedAt: new Date().toISOString(), status: "error", models: [], error: error instanceof Error ? error.message : "external status request failed" }; } }));
}
async function query(provider, credentials, force = false, writeCache = true, draftSecret = null) {
  // 每个供应商独立控制自动查询间隔；手动刷新通过 force 跳过缓存，草稿测试显式禁用缓存。
  const cacheMs = Math.max(0, Number(provider.queryIntervalMinutes ?? DEFAULT_QUERY_INTERVAL_MINUTES)) * 60_000;
  const existing = cache.get(provider.id); if (writeCache && !force && cacheMs > 0 && existing && Date.now() - existing.at < cacheMs) return existing.value;
  ...
  // 查询时重新解析并固定到已经过公网校验的 IP，阻断保存后 DNS 重绑定到内网地址。
  ...
  if (writeCache) cache.set(provider.id, { at: Date.now(), value }); return value;
}
...
export function apply(ctx) {
  ...
  const registration = {
    ...
    try { const url = new URL(req.url || "/", "http://local"); const config = await loadConfig();
      ...
      if (req.method === "GET" && url.pathname === "/dsh-balance-quota/external-status") return json(res, 200, { ok:true, sources:await externalSummary(config, url.searchParams.get("force") === "1", url.searchParams.get("source")) });
       if (req.method === "POST" && url.pathname === "/dsh-balance-quota/external-status-preview") return json(res, 200, { ok:true, ...(await previewExternalStatusSource(await body(req))) });
      ...
      if (req.method === "POST" && url.pathname === "/dsh-balance-quota/external-status-source") { const input = await body(req); const source = await validateExternalStatusSource(input); await mutateConfig(async current => { current.externalStatusSources = normalizeExternalSources([...(Array.isArray(current.externalStatusSources) ? current.externalStatusSources : []).filter(item => item.id !== source.id && (!source.providerId || item.providerId !== source.providerId)), source]); await saveConfig(current); }); externalStatusCache.delete(source.id); return json(res, 200, { ok: true, source }); }
       if (req.method === "POST" && url.pathname === "/dsh-balance-quota/provider/test") { const input=await body(req); const provider=await validateProvider(input); const draftSecret=typeof input.apiKey === "string" && input.apiKey.length > 0 ? input.apiKey : null; if (!draftSecret && !input.credentialRef) throw new HttpError(400, "API Key 或凭据引用不能为空"); return json(res,200,{ok:true,result:await query(provider, ctx.credentials, true, false, draftSecret)}); }
